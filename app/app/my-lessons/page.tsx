...
/** Stable color classes per timetable (by id) for card border + badge */
...
export default function MyLessonsPage() {
	...
	// Filter by timetables: empty = show all; non-empty = show only selected
	const [selectedTimetableIds, setSelectedTimetableIds] = useState<string[]>([])
	// Available timetables from API (id, name); used for filter checkboxes
	const [availableTimetables, setAvailableTimetables] = useState<{ id: string; name: string | null }[]>([])
	// Custom date range (YYYY-MM-DD); used when range === "custom"
	...
	const load = useCallback(() => {
		...
		return fetch(`/api/app/my-lessons?${params.toString()}`)
			...
			.then((json: { lessons?: LessonItem[]; availableTimetables?: { id: string; name: string | null }[] }) => {
				if (json) {
										...
										<DropdownMenuCheckboxItem
											...
											onCheckedChange={(checked) => {
												setSelectedTimetableIds((prev) => {
													if (selectedTimetableIds.length === 0) {
														// "All" was selected: turning one on means only this one
