...
export default function ClubLessonTypesPage() {
	const router = useRouter()
	const [clubData, setClubData] = useState<ClubData | null>(null)
	const [types, setTypes] = useState<GroupLessonType[]>([])
	...
	const loadTypes = useCallback(() => {
		return fetch("/api/club/group-lesson-types")
			...
			.then((json) => {
				if (json?.group_lesson_types) setTypes(json.group_lesson_types)
			})
	}, [])

	const loadData = useCallback(() => {
		return Promise.all([loadClub(), loadTypes()]).catch((e) =>
	...
	function openCreate() {
		setEditingType(null)
		setFormGroupId(clubData?.groups?.[0]?.id ?? "")
		setFormName("")
		setFormDuration("45")
		setDialogOpen(true)
	}
	...
	// Group types by group_id for display
	...
	const groupOrder = clubData?.groups ?? []
